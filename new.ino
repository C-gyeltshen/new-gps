...
bool sendLocation(float lat, float lng, float speed,
  ...
  // Wait until the modem signals the server closed the connection (Connection: close)
  // This exits immediately when "CLOSED" arrives rather than blocking 30 s flat.
  String result = waitForToken("CLOSED", 30000);

  Serial.println(F("[SERVER RESPONSE]"));
  Serial.println(result);

  // Connection may already be closed by the server; ignore the error if so.
